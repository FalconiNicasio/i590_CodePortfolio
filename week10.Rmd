...
Model 2: Y = 3+5x+ϵ, ϵ ∼ N(0,x^2^)

Model 3: Y = 3+5x2+ϵ, ϵ ∼ N(0,25)
...
plot(y ~ x, data = sim_data_1, col = "grey", pch = 20,
     main = "Data from Model 1") #pch is point shape

fit_1 = lm(y ~ x, data = sim_data_1) #lm = linear model

abline(fit_1, col= "darkorange", lwd = 3) #abline = add line, lwd = line width
```

We not plot a fitted versus residuals plot, Note, this is residuals on the *y*-axis despite the ordering in the name. Sometimes you will see this called a residuals versus fitted, or residuals versus predicted plot.

```{r}
plot(fitted(fit_1), resid(fit_1), col = "grey", pch = 20,
     xlab = "Fitted", ylab = "Residuals", main = "Data from Model 1")

abline(h = 0, col = "darkorange", lwd = 2) #h = the y-value(s) for horizontal line(s)
```

We should look for two things in this plot.

* At any fitted value, the mean of the residuals shuold be roughly 0. If this is the case, the *linearity* assumption is valid. For this reason, we generally add a horizontal line at *y* = 0 to emphasize this point.
* At every fitted value, the spread of the residuals should be roughly the same. If thisis the case, the *constant variance* assumption is valid.

Here we see this is the case for both.

To get a better idea of how a fitted versus residuals plot can be useful, we will simulate fromm models with violated assumptions.



**Model 2** is an exmaple of **non-constant variance**. In this case, the variance is larger for larger values of the predictor variable *x*.
...
This actually is rather easy to see here by adding the fitted line to a scatterplot. This is because we are only performing simple linear regression. With multiple regresssion, a fitted versus residuals plot is a necessity, since adding a fitted regression to a scatterplot isn't exactly possible.
...
On the fitted versus residuals plot, we see two things veryt clearly. For any fitted value, the residuals seem roughly centered at 0. This is good! The linearity assumption is not violated. However, we also see very clearly, that for larger fitted values, the spread of the residuals is larger. This is bad! The constant variance assumption is violated here. 

Now we will demonstrate a model whihc does not meet the linearity assumption. **Model 3** is an example of a model where *Y* is not a linear combiation of the predictors. In this case the predictor is *x*, but the model uses *x*^2^. (We'll see later that this is something that a "linear" model can deal with. The fix is simple, just make *x*^2^ a predictor!)
...
While a fitted versus residuals plot can give us an idea about homoscedasticity, sometimes we would prefer a more formal test. There are many tests for constant variance, but here we will present one, the [Breusch-Pagan Test](https://en.wikipedia.org/wiki/Breusch%E2%80%93Pagan_test). The exact details of the test will be omitted here, but importantly the null and alternative can be considered to be,
* *H*~0~: Homoscedasticity. The errors have constant variance about the true model
* *H*~1~: Heteroscedasticity. The errors have non-constant variance about the true model. 
...
Lastly, for ```fit_3``` we again see a large p-value, so we do not reject the null of homoscedasticity, which matches our findigs with a fitted versus residuals plot.
...
```{r}
par(mfrow = c(1, 3)) #creates parameter(par) for creating 3 plots on a 1 X 3 matrix filled in by row(mfrow)
...
Above are histograms for each of the three regression we have been considering. Notice trhat the first, for ```fit_1``` appears very normal. The third, for ```fit_3```, appears to be very non_normal. However ```fit_2``` is not as clear. It does have a rough bell shape, however, it also has a very sharp peak. For this reason we will usually use more powerful tools such as **Q-Q plots** and the **Shapiro-Wilk test** for assessing the normaility of errors.


### 13.2.4 Q-Q Plots

Another visual method for assessing the normality of errors, which is more powerful than a histogram, is a normal qunatile-quantile plot, or **Q-Q plot** for short.

In ```R``` these are very easy to make. The ```qqnorm()``` function plots the pionts, and the ```qqline()``` function adds the necessary line. We create a Q-Q plot for the residuals of ```fit_1``` to check if the errors could truly be normally distributed.
...
The calculations required to create the plot vary depnding on the implementation, but essentially the *y*-axis is the sorted data (observed, or sample qunatiles), and the *x*-axis is the values we would expect if the data did come from a normal distribution (theoretical quantiles). 

The [Wikipedia page from Normal Probability plots](http://en.wikipedia.org/wiki/Normal_probability_plot) gives details on how this is implemented in ```R``` if you are interested.
...
We can then verify that it is essentialy equivalent to using ```qqnorm()``` and ```qqline()``` in ```R```
...
Since this data **is** sampled from a normal distribution, these are all, by definition, good Q-Q plots. The points are "close to the line" and we could conclude that this data could have been sampled from a normal distribution. Notice in the first plot, one point is *somewhat* far from the line, but just one point, in combination with the small sample size, it not enough to make us worried. We see with the large sample size, all the points are rather close to the line.

Next, we simulate data from a *t* distribution with small degrees of freedom, for different sample sizes.
...
Recall, that as the degrees of feedom for a *t* distribution become larger, the distribution becomes more and more similar to a normal. Here, suing 4 degrees of freedom, we have a distribution that is somewhat normal, it is symmetrical and roughly bell-shaped, however it has "fat tails." This presents itself clearly in the third panel. While many of the points are close to the line, at the edges, there are large discrepancies. This indicates that the values are to small (negative) or too large (positive) compared to what we would expect for a normal distribution. So for the sample size of ```100```, we would conclude that the normality assumption is violated. (If these were residuals of a model.) For sample sizes of ```10``` and ```25``` we may be suspicious, but not entirely confident. Reading Q-Q plots, is a bit of an art, not completely a science.
...
For a better understanding of which Q-Q plots are "good," repeat the simulations above a number of times (without setting the seed) and pay attention to the differences between those that are simulated from normal, and those that are not. Also consider different smaple sizes and distribution parameters.
...
```{r}
qqnorm(resid(fit_3), main = "Normal Q-Q Plot, fit)3", col = "darkgrey")
qqline(resid(fit_3), col = "dodgerblue", lwd = 2)
```

Lastly, for ```fit_3```, we again have a suspect Q-Q plot, We would probably **not** believe the rorrs follow a normal distribution.


### Shapiro-Wilk Test

Histograms and Q-Q Plots give a nice visual representation of the redisuals distribution, however if we are interested in formal testing, there are a number of options available. A commonly used test is the **Shapiro-Wilk test**, which is implemented in ```R```.
...
This gives us the value of the rest statistic and its p-value. The null hypothesis assumes the data were sampled from a normal distribution, thus a small p-value indicates we blieve there is only a small probability the data could have been sampled from a normal distribution.

For details, see: [Wikipedia: Shaprio-Wilk tst](https://en.wikipedia.org/wiki/Shapiro-Wilk_test).
...
Returning again to ```fit_1```, ```fit_2```, ```fit_3```, we see the result of running ```shapiro.test()``` on the residuals of each, returns a result for each that matches for decisions based on the Q-Q plots.
...
The blue solid line in each plot is a regression fit to the 10 original data points stored in ```ex_data```. The dashed orange line in each plot is the result of adding a single point to he original data in ```ex_data111. This additional point is indicated by the circled point.
...
Lastly, the added point in the third plot has *large* effect on the slope, which is now:
...
A data point with higfh **leverage*, is a data point that *could* have a large influence when fitting the model.
...
Here we've created some multivariate data. Notice that we have plotted the *x* values, not the *y* values. The red point is (7, 3) which is the mean of ```x^2^``` respectively.

We could calculate the leverages using the expressions defines above. We first create the *X* matrix, then calculate *H* as defined, and extract the diagonal elements.

```{r}
X = cbind(rep(1, 8), lev_ex$x1, lev_ex$x2) #column bind to create matrix
H = X %*% solve(t(X) %*% X) %*% t(X) # %*% = matrix multiplication t() matrix transpose
diag(H)
```
...
```{r}
lev_fit = lm(y ~ ., data = lev_ex) # . means everything else x1 and x2
hatvalues(lev_fit)
```
...
We see that the original ```y``` value is 11. We'll create a copy of the data, and modiofy this piont to have a ```y``` value of ```20```.
...
Npotice the **large** changes in the coefficients. Also notice that each of the coefficients has changfed in some way. Note that the leverages of the points would not have changed, as we have not modified any of the x values.
...
Returning again to our three plots, each with an added point, we can calculate the residuals and standardized residuals for each. Standardized reiduals can be ovbtained in ```R``` by using ```rstandard()``` where we would normally use ```resid()```
...
In the second plot, we see that there are no points with large standrdized residuals.
...
A Cook's Distance is often considered large if D~i~ > 4/n
...
And, as expected, the added point in the third plot, with high leverage and large residual is considered influential!
...
Last chapter we fit an additive regression to the ```mtcars``` data with ```mp``` as the response and ```hp``` and ```am``` as predictors. Lets perform some diagnostics on this model.
...
The fitted versus residuals plot looks good. We don't see any obvious pattern, and the variance looks roughly constant. (Maybe a little larger for lage fitted values, but not rnough to worry about.)

```{r}
bptest(mpg_hp_add)
```

The Breusch-Pagan test verfies this, at least for a small α value.
...
We find two influential point. Interestingly, they are **very** different cars.
...
It seems there isn't much of a change in the coefficients as a result of removing the supposed inflential points. Notice we did not create a new dataset to accomplish this. We instead used the ```subet``` argument to ```lm()```. Think about what the code ```cd_mpg_hp_add <= 4 / length(cd_mpg_hp_add)``` does here.
...
Notice that, calling ```plot()``` on a variable which stores an object created by ```lm()``` outputs four diagnostic plots by default. Use ```?plot.lm``` to learn more. The first two shuold already be familiar.


### 13.4.2 Suspect Diagnostics


Let's consider the model `big_model` from last chapter which was fit to the `autompg` dataset. It used `mpg` as the reponse, and considered many interaction terms between the predictors `disp`, `hp`, and `domestic`.
...
We've now seen that sometimes modifying the data can fix issues with regression. However, next chapter, insteaed of modifying the data, we will modify the model via *transformations*.
