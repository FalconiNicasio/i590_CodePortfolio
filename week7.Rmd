...
```
Not useful because the drivetrain and class of the vehicle is not a relationship that matters in this case study. We are interested in miles per gallon and neither of these data points shows a relationship to how the efficiency of of a vehicle.

## 3.3 Aethetic mappings
...
ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_point() +
  geom_point(data = ds, color = 'red', size = 2)
```
...
1. What's gone wrong with this code? Why are trhe points not blue?
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, color = "blue"))
```

The ```color``` attribute is within the ```aes()```. The correct code:
```
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue")
```
...
str(mpg)
```
Categorial : manufacturer, model, trans, drv, fl, class, year, cyl
Continuous : displ, cty, hwy
You can check ```mpg``` and see if the variables are numeric (```int``` or ```num```) or if they are not (```chr```). If they are not then it is categorical, if its numeric you have to check whether its continuous or discrete data. 
...
```
For aesthetics continuous variables when assigned to color are given a spectrum on a color pallete vs for categorical variables it was discrete binned categories. For size, it is similar to color that the size of points is on a spectrum of sizes. For shape, it was not able to do a continuous variable so there needs to be discrete categories. (can't do half a shape)
...
Multiple changes are made with multiple legends.
...
```stroke``` adjust the thickness of the border of the point. *NOTE*, Works on shapes
...
Creates a logical legend TRUE or FALSE and colors the data appropiately.
...
The empty spots in the facet grid are the same empty values as the plot comparing drv and cyl that have no data points.
...
The ```.``` , based on where its at, forces the facet to be plotted either on the x axis or the y axis.
...
The advantages to using faceting is that you can compare multiple variables on the same axis separately which can show more detail rather than the color aesthetic where data points can overlap and make it unclear. The disadvantages is that it takes up a lot of space and the plots can be more difficult to compare observations with each other because they are on separate plots rather than the same. If I had a larger data set I think faceting would be even more useful because the color aesthetic would have much more overlap. 

5. Read ```?facet_wrap```. What does ```nrow``` do? What does ```ncol``` do? What other options control the layout of the individual panels? Why doesn’t ```facet_grid()``` have nrow and ncol arguments?

```nrow``` controls how many rows of plots you will have, and ```ncol``` controls how many columns you will have. ```facet_grid()``` doesn't have ```nrows``` or ```ncol``` because it takes specific arguments for rows and columns already. 

6. When using ```facet_grid()``` you should usually put the variable with more unique levels in the columns. Why?

For better use of space. Landscape

## 3.6 Geometric objects

How are these two plots similar?
```{r,echo=FALSE, fig.width=3, fig.height=2,fig.show='hold'}
#left
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy))
#right
...
It removed the legend and you removed it earlier in the chapter to hide it to give the viewer something to think about by showing just the plot without the legend
...
```
No they will look the same because they have the same data and mapping. The first one is just cleaner and simplier.
...
```geom_col``` expects an x and a y statement and uses the y for the height of data point on the plot while ```geom_bar``` only asks for an x statement and uses stat_count() to count the frequency of the variable.
3. Most geoms and stats come in pairs that are almost always used in concert. Read through the documentation and make a list of all the pairs. What do they have in common?
They always have each other as their default
...
Without ```group = 1``` then the proportion plot would have every variable on the x axis as 1.
...
There is some overlapping on this plot and it can be improved by using ```position = "jitter"``` or ```geom_jitter```
```{r}
ggplot(mpg) +
...
```geom_jitter``` adds random variation to the locations of points on the graph which reduces overlapping but reduces the true accuracy of the data points. ```geom_count``` adds sizes to the points relative to the number of observations but can cause more overlapping. 
4. What’s the default position adjustment for ```geom_boxplot()```? Create a visualisation of the ```mpg``` dataset that demonstrates it.
The default position for ```geom_boxplot()``` is ```position_dodge2``` which moves the geom horizontally to avoid overlapping without changing the vertical position. 
```{r}
ggplot(mpg) +
...
1. Turn a stacked bar chart into a pire chart using ```coord_polar()```.
```{r}
...
```labs()``` stands for labels and you can add axis titles, plot titles, and a caption. 
...
```coord_fixed()``` ensures the line ```geom_abline()``` is at a 45 degree angle to make it easier to compare the two variables by eye. 
